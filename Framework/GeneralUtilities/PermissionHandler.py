...
async def check_permissions(ctx, embed, module, command, shouldCheckForWizard=False, shouldCheckForModuleEnabled=True,
	...
	elif shouldCheckForBannedModule and await CommandAccess.check_user_is_banned_from_module(ctx.message.author.mention, module, guild_id):
		embed.title = "Cannot use this module"
		embed.description = "You do not have access to use this module."
		failedPermissionCheck = True
	elif shouldCheckForBannedCommand and await CommandAccess.check_user_is_banned_from_command(ctx.message.author.mention, command, guild_id):
