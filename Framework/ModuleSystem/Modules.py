...
class ModuleSystem(commands.Cog):
	...
	async def module_info(self, ctx, module=None):
		...
		if await CommandAccess.check_user_is_banned_from_command(ctx.message.author.mention, "moduleInfo"):
	...
	async def toggle_module_state(self, ctx, module=None):
		...
		if await CommandAccess.check_user_is_banned_from_command(ctx.message.author.mention, "toggleModuleState"):
			embed.title = "Cannot use this command"
			embed.description = "You do not have permission to use this command."
